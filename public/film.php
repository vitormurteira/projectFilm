...
$title = isset($_POST['newTitle']) ? strip_tags(trim($_POST['newTitle'])) : '';

//NB: le strip_tag évite les injections

//2. RECUPERATION DE LA DONNEE "synopsis":

$synopsis = isset($_POST['newSynopsis']) ? strip_tags(trim($_POST['newSynopsis'])) : '';

//3. RECUPERATION DE LA DONNEE "affiche":

$affiche = isset($_POST['newAffiche']) ? strip_tags(trim($_POST['newAffiche'])) : '';

//4. RECUPERATION DE LA DONNEE "date de sortie ou release_date":

$release_date = isset($_POST['newReleaseDate']) ? strip_tags(trim($_POST['newReleaseDate'])) : '';

//5. RECUPERATION DE LA DONNEE "date de telechargement ou register_date":

$registery_date = isset($_POST['newRegisterDate']) ? strip_tags(trim($_POST['newRegisterDate'])) : '';

//6. RECUPERATION DE LA DONNEE "description longue":

$description = isset($_POST['newDescription']) ? strip_tags(trim($_POST['newDescription'])) : '';

//7. RECUPERATION DE LA DONNEE "categories":

$categories = isset($_POST['newCategories']) ? strip_tags(trim($_POST['newCategories'])) : '';

//8. RECUPERATION DE LA DONNEE "acteurs":

$actors = isset($_POST['newActors']) ? strip_tags(trim($_POST['newActors'])) : '';

//9. RECUPERATION DE LA DONNEE "nationalité":

$nationality = isset($_POST['newNationality']) ? strip_tags(trim($_POST['newNationality'])) : '';

//10. RECUPERATION DE LA DONNEE "support":

$support = isset($_POST['newSupport']) ? strip_tags(trim($_POST['newSupport'])) : '';

//11. RECUPERATION DE LA DONNEE "chemin":

$path = isset($_POST['newPath']) ? strip_tags(trim($_POST['newPath'])) : '';
...
else if (check_date_format("d.m.Y", $release_date)){
	$formValid = false;
	echo "Release date should comply with the following design: dd/mm/yyyy";
}
...
else if (check_date_format("d.m.Y", $registery_date)){
	$formValid = false;
	echo "Register date should be introduced following the french format";
}
